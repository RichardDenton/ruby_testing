...
describe Cat do
  ...
  subject(:felix) { described_class.new('Felix', 'kitty', 'black') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when method is shared with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(felix).to_not be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(felix).to be_hiding
  end
end
