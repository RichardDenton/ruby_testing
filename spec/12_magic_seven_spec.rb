...
describe MagicSeven do
  ...
  describe '#subtract_four' do
    context 'when previous step is 6' do
      it 'returns 2' do
        previous_step = 6
  ...
  describe '#subtract_random_number' do
    context 'when random number is 4 and previous step is 12' do
      subject(:four) { described_class.new(4) }
      it 'returns 8' do
        previous_step = 12
        result = four.subtract_random_number(previous_step)
  ...
  describe '#play' do
    ...
    context 'when the random number is 15' do
      subject(:fifteen) { described_class.new(15) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = fifteen.play
        expect(result).to eq(7)
    ...
    context 'when the random number is 2' do
      subject(:two) { described_class.new(2) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = two.play
        expect(result).to eq(7)
