...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        valid = game_verify.verify_input(min, max, valid_input)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        min = game_verify.instance_variable_get(:@minimum)
        max = game_verify.instance_variable_get(:@maximum)
        valid = game_verify.verify_input(min, max, invalid_input)
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_sixhundred) { described_class.new(100, 600) }
      it 'returns 9' do
        max = game_sixhundred.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_turn_order) { described_class.new(1, 100) }
    ...
    it 'increases guess_count by one' do
      expect { game_turn_order.display_turn_order(binary_search_turn) }.to change { game_turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_turn).to receive(:make_guess).once
      game_turn_order.display_turn_order(binary_search_turn)
    end
