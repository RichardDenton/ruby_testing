...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match { |number| number >= 0 and number <= 9 }
      end
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:not_ended) { described_class.new(5, 4) }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(not_ended).not_to be_game_over
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      subject(:game2) { described_class.new(4, '4', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game2.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
