...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:answer) { double('answer', value: 5) }
      subject(:game_won) { described_class.new(1, 9, answer, answer.value) }
      it 'is game over' do
    ...
    context 'when guess and random_number are not equal' do
      let(:answer) { double('answer', value: 5) }
      subject(:game_not_won) { described_class.new(1, 9, answer, answer.value + 1) }
      it 'is not game over' do
        expect(game_not_won).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'does not update max' do
        start_max = low_guess_game.max
        low_guess_game.update_range
        new_max = low_guess_game.max
        expect(new_max).to eq(start_max)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guess_seven) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        guess_seven.update_range
        new_min = guess_seven.min
        expect(new_min).to eq(8)
      end

      it 'does not update max' do
        guess_seven.update_range
        new_max = guess_seven.max
        expect(new_max).to eq(8)
      end
    end
  end
